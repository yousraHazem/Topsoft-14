(function($) {
    $.fn.jqPuzzle = function(settings, texts) {
        ...
        var defaults = {
            ...
            hole: 16,
            // initial hole position [1 ... rows*columns]
            shuffle: false,
            // initially show shuffled pieces [true|false]
            numbers: false,
            // initially show numbers on pieces [true|false]
            language: 'en',
            // language for gui elements [language code]
            // display additional gui controls
            control: {
                shufflePieces: true,
                // display 'Shuffle' button [true|false]
                confirmShuffle: true,
                // ask before shuffling [true|false]
                toggleOriginal: true,
                // display 'Original' button [true|false]
                toggleNumbers: true,
                // display 'Numbers' button [true|false]
                counter: true,
                // display moves counter [true|false]
                timer: true,
                // display timer (seconds) [true|false]
                pauseTimer: false
                // pause timer if 'Original' button is activated 
                // [true|false]
            },
            // perform actions when the puzzle is solved sucessfully
            success: {
                fadeOriginal: true,
                // cross-fade original image [true|false]
                callback: true,
                ...
                callbackTimeout: 300
                // time in ms after which the callback is called
            },
            // animation speeds and settings
            animation: {
                shuffleRounds: 3,
                // number of shuffle rounds [1 ... ]
                shuffleSpeed: 800,
                // time in ms to perform a shuffle round
                slidingSpeed: 200,
                // time in ms for a single move
                fadeOriginalSpeed: 600 
                // time in ms to cross-fade original image
            },
            // additional style information not specified via css
            style: {
                gridSize: 2,
                // space between two pieces in px
                overlap: true,
                // if true, adjacent piece borders will overlap
                // applies only if gridSize is set to 0
                backgroundOpacity: 0.1 
                // opacity of the original image behind the pieces
        ...
        if (settings && !settings.hole && (settings.rows || 
            settings.cols)) {
            settings.hole = (settings.rows || defaults.rows)
                * (settings.cols || defaults.cols);
        }
        // extend the user-defined settings object with
        // default settings.
        settings = $.extend(true, {}, defaults, settings);
        // extend the user-defined texts object with current language texts
        texts = $.extend((i18n[settings.language] ||
            i18n[defaults.language]), texts);
        ...
        if ((hole > (rows * cols)) || (hole < 1)){
            hole = rows * cols;
            hole--; 
        }
        // zero-based index
        // animation speed = 0 doesn't work
        if (animation.slidingSpeed < 1) 
            animation.slidingSpeed = 1;
        if (animation.shuffleSpeed < 1) 
            animation.shuffleSpeed = 1;
        if (animation.fadeOriginalSpeed < 1) 
            animation.fadeOriginalSpeed = 1;
        // keep shuffle rounds within limits
        if (animation.shuffleRounds < 1) 
            animation.shuffleRounds = 1;
        ...
        var checkSolution = function($pieces) {
            ...
            for (var i = 0; i < $pieces.length; i++) {
                ...
                // check if current position match target 
                //(index) position
                if ($pieces.eq(i).attr('current') != pieceIndex) 
                    return false;
            }
            return true;
        };
        ...
        var checkOrder = function(numbersArray) {
            ...
            for (var i = 1; i <= (rows * cols - 1); i++) {
                for (var j = (i + 1); j <= (rows * cols); j++) {
                    product *= ((numbersArray[i - 1] 
                        - numbersArray[j - 1]) / (i - j));
                }
            }
        ...
        // (internet explorer returns keywords)
        ...
        return this.filter('img').each(function() {
            ...
            var $dummyWrapper = $('<div/>').addClass(
                'jqp-wrapper').append($dummyPiece);
            var $dummyGui = $('<div/>').attr('class',
                $srcImg.attr('class') || '') // transfer classes
            .addClass('jqPuzzle').append($dummyWrapper);
            ...
            var computedStyles = {
                gui: {
                    ...
                    padding: {
                        left: parseInt($dummyGui.css('padding-left'))
                            || 0,
                        right: parseInt($dummyGui.css('padding-right'))
                            || 0,
                        top: parseInt($dummyGui.css('padding-top'))
                            || 0,
                        bottom: parseInt($dummyGui.css('padding-bottom'))
                            || 0
                    }
                },
                wrapper: {
                    border: getBorderWidth($dummyWrapper),
                    padding: parseInt($dummyWrapper.css('padding-left')) 
                        || 0
                },
                piece: {
            ...
            // to be able to get its real width/height
            $srcImg.one('load', function() {
                // overlap piece borders,
                // if there is no margin between pieces
                // this way, piece borders will not be doubled
                var overlap = (style.gridSize === 0 && style.overlap);
                // total space of piece borders and grid lines, 
                // which will cover parts of the image
                var coveredWidth = cols * (2 * computedStyles.piece.border)
                    + (cols - 1) * style.gridSize;
                var coveredHeight = rows * (2 * computedStyles.piece.border)
                ...
                // make sure to get the original image size,
                // not scaled values
                // in mozilla, width() and height()
                // do not work with hidden elements
                $srcImg.css({
                    height: 'auto',
                    visibility: 'visible'
                    width: 'auto',
                ...
                var width = Math.floor(($srcImg.width() -
                    coveredWidth) / cols);
                var height = Math.floor(($srcImg.height() -
                    coveredHeight) / rows);
                ...
                // total width/height of a piece 
                // (including piece border and 1 grid size)
                var totalPieceWidth = width + 2 * computedStyles.piece.border + 
                    style.gridSize;
                var totalPieceHeight = height + 2 * computedStyles.piece.border + 
                    style.gridSize;
                // handle internet explorer quirks mode box model
                var boxModelHack = {
                    piece: $.boxModel ? 0 : 2 * computedStyles.piece.border,
                    wrapper: $.boxModel ? 0 : 2 * (computedStyles.wrapper.border +
                ...
                var shuffle = function(rounds, speed) {
                    ...
                    // the function was triggered by a user event (button click)
                    if (speed) {
                        ...
                        // reset gui
                            ...
                            // fade out original
                            $background.fadeTo(animation.fadeOriginalSpeed,
                                style.backgroundOpacity, function() {
                    ...
                    // reset display
                    if ($display) 
                        $display.removeClass('jqp-disabled');
                    if ($counter) 
                        $counter.val(moves);
                    if ($timer) 
                        $timer.val(seconds);
                    ...
                    while (i < rounds) {
                        ...
                        // but keep hole at initial position
                            ...
                            // save the value at this index as the next number
                            // in the current order
                            shuffles[i][j] = choices[randomIndex];
                            // remove this value from the choices array
                            // (reducing its length)
                            ...
                            if (speed === undefined) {
                                ...
                                $piece.animate({
                                  ...
                                  top: offset.top
                                    ...
                                    animCounter++;
                                    if (animCounter == animation.shuffleRounds *
                                        (rows * cols - 1)) {
                                        lock = false;
                                        animCounter = 0;
                                    }
                ...
                for (var i = 0; i < rows; i++) {
                    for (var j = 0; j < cols; j++) {
                        ...
                        var bgLeft = -1 * (j * totalPieceWidth +
                            computedStyles.piece.border);
                        var bgTop = -1 * (i * totalPieceHeight + 
                            computedStyles.piece.border);
                        // recalc if overlap
                            if (overlap) {
                ...
                if (control.counter || control.timer) {
                    ...
                    $display = $('<span/>').css('cursor', 'default').
                        appendTo($controls);
                    ...
                    if (control.counter) {
                         $counter = $protoField.clone().
                            appendTo($display).after(texts.movesLabel + ' ');
                    }
                    // create timer component
                    if (control.timer) {
                        $timer = $protoField.clone().
                            appendTo($display).after(texts.secondsLabel);
                    }
                    // disable display if the puzzle is not shuffled yet
                    if (!settings.shuffle) 
                        $display.addClass('jqp-disabled');
                }
                ...
                // panel which holds controls and credits,
                ...
                var $panel = $('<div/>').css({
                    ...
                    width: fullWidth + 2 * (computedStyles.wrapper.padding + 
                        computedStyles.wrapper.border)
                }).append($credits).append($controls);
                // full gui (including wrapper and panel).
                // transfer classes.
                var $gui = $('<div/>').attr('class', $srcImg.attr('class') || '')
                    .addClass('jqPuzzle') // always assign class jqPuzzle
                    .css({
                        display: 'block',
                        height: fullHeight + 2 * (computedStyles.wrapper.padding +
                            computedStyles.wrapper.border) + boxModelHack.gui.height,
                        overflow: 'hidden',
                        textAlign: 'left',
                        width: fullWidth + 2 * (computedStyles.wrapper.padding + 
                            computedStyles.wrapper.border) + boxModelHack.gui.width
                ...
                $buttons.mousedown(function() {
                    if (!$(this).is('.jqp-disabled')) 
                        $(this).addClass('jqp-down');
                });
                ...
                $pieces.click(function() {
                    ...
                    if (Math.abs(source.row - dest.row) + 
                        Math.abs(source.col - dest.col) != 1) {
                        lock = false;
                        return false;
                    }
                ...
                if (control.toggleOriginal) $originalButton.click(function() {
                    // do nothing, if disabled
                    if ($originalButton.is('.jqp-disabled')) 
                        return false;
                    // do nothing, if locked
                    if (lock) 
                        return false;
                ...
                if (control.toggleNumbers) $numbersButton.click(function() {
                    // do nothing, if disabled
                    if ($numbersButton.is('.jqp-disabled')) 
                        return false;
                ...
                var finishGame = function() {
                    if (success.fadeOriginal) {
                        ...
                        url = url + "?moves=" + moves + "&time=" + seconds + 
                            "&score=" + score;
                ...
                // call a user-defined callback after a timeout,
            ...
            // img load
            // unfortunately, image load does not fire consistently
            // across browsers (especially with cached images)
    ...
    $(document).ready(function() {
        $('img.jqPuzzle').each(function() {
            ...
            */
            var microFormat = /\bjqp(-[a-z]{2})?-r(\d)-c(\d)(-h(\d+))?(-s(\d+))?
                (-[A-Z]+)?\b/;
