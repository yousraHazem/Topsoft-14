...
# This method to provide the child with a list of friends' events.
# child - the child currently signed in.
# @event - active record of childrens' friends' events.
# Returns @event which is the active record of the child's friends events.
# Time complexity: O(n).
#Authors: Nouran Mamdouh.
 
  def view_my_friends_events
    child = Child.where("registered_user_id=#{current_user.id}").first
