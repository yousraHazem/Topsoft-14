class PollQuestionsController < ApplicationController
  ...
  # creating a defult answer  
    @poll_question.poll_answers.build
  end

  # function to increment the counter of each answer when voted.
  def calc
    p = PollAnswer.find_by_content params[:poll]
    @poll_question = PollQuestion.find(p.poll_question_id)
    m = p.counter
    m = m.to_i + 1
    p.update_attributes counter: m
    render action: 'show', status: :created, location: @poll_question
  end
